...
def main():
    parser = argparse.ArgumentParser(description="Refine Alpaca prompts using an LLM")
    ...
    if args.base_url and args.api_key and args.api_model_name:
        logging.info("Using API model for Alpaca refinement.")
        refiner.run_refinement_api(max_new_tokens=args.max_new_tokens)
    else:
        logging.info("Using local HF model for Alpaca refinement.")
